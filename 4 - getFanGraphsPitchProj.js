/**
 * FILE: getFanGraphsPitchProj.gs
 * PURPOSE: Fetches pitching projections from multiple FanGraphs 
...
const FG_PROJ_PITCH_SHEET = '_FG_PROJ_P';
...
function getFanGraphsPitchProj() {
  ...
  if (!currentYear) {
    Logger.log('getFanGraphsPitchProj: CURRENT_YEAR not found. Aborting.');
    return;
  }

  Logger.log(`getFanGraphsPitchProj: Fetching projections for ${currentYear}...`);
  ...
  for (let groupName in FG_PITCH_PROJ_GROUPS) {
    FG_PITCH_PROJ_GROUPS[groupName].forEach(variant => {
      requestQueue.push({
        url: `${FG_PROJ_BASE_URL}?pos=all&stats=pit&type=${variant.id}`,
        ...FG_PROJ_OPTIONS
      });
  ...
  responses.forEach((response, idx) => {
    ...
    // ADDED SAFETY NET: try...catch block for parsing
    let json;
    try {
      json = JSON.parse(response.getContentText());
    } catch (e) {
      Logger.log(`Failed to parse JSON for ${meta.groupName} ${meta.sysType}. Skipping.`);
      return; 
    }
    ...
    if (data.length > 0) {
      ...
      data.forEach(row => {
        ...
        if (!idCache[cacheKey]) {
          idCache[cacheKey] = resolveMasterId(maps, fgId, mlbId, pName, 'getFanGraphsPitchProj', teamAbbr);
        }
  ...
  if (allObjects.length > 0) {
    ...
    writeToData(FG_PROJ_PITCH_SHEET, outputData);
    Logger.log(`getFanGraphsPitchProj: Wrote ${outputData.length - 1} projection rows to ${FG_PROJ_PITCH_SHEET}`);
    ...
    successfulUpdates.forEach(updateKey => {
      ...
      if (range) {
        ...
        Logger.log(`getFanGraphsPitchProj: Named range ${rangeName} not found on sheet.`);
      }
    });
    
    flushIdMatchingQueue();
  } else {
    Logger.log('getFanGraphsPitchProj: No projection data returned.');
  }
}