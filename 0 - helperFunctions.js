 ...
 *
 * READS FROM: Named ranges in primary sheet (Settings tab)
 * Yahoo API (via yahooAuthentication.gs)
 * WRITES TO:  Primary workbook, Data workbook, Archive workbook
 ...
 * Primary  — Display sheets, dashboards, calculation layers.
 ...
 * writeToSheet(sheetName, data)    → Primary workbook
 ...
 * in the primary sheet Settings tab.
...
/**
 * Writes data to a named sheet in the PRIMARY workbook.
...
function writeToSheet(sheetName, dataArray) {
  _writeToWorkbook(getPrimarySS(), sheetName, dataArray, 'Primary');
}
...
/**
 * Writes the current date/time to a named range in the primary sheet.
 ...
 * COUNT_CELLS_PRIMARY  — cell count in primary workbook
...
function spreadsheetCounts() {
  ...
  const targets = [
    { getter: getPrimarySS, rangeName: 'COUNT_CELLS_PRIMARY' },
 ...
 *
 * @param {Spreadsheet} ss - The primary spreadsheet (passed in to avoid
 * redundant getPrimarySS() calls from callers)
 */
